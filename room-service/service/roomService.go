...
type RoomRepository interface {
	...
	UpdateRoomStatus(ctx context.Context, id string, status models.RoomStatus) error
}
...
// Update room status
func (s *RoomService) UpdateRoomStatus(ctx context.Context, id string, status models.RoomStatus) error {
	if !status.IsValid() {
	...
	if status == models.StatusMaintenance && currentRoom.Status == models.StatusBooked {
